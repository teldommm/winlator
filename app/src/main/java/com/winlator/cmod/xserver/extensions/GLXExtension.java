...
public class GLXExtension implements Extension {
    ...
    private class GLXContext {
        ...
        GLXContext(int id, int visualId, FBConfig fbconfig, int screen, int renderType,
                   int shareList, boolean isDirect, int majorGL, int minorGL,
                   int flags, int profileMask) {
            this.id = id;
            this.visualId = visualId;
            this.fbconfig = fbconfig;
            this.screen = screen;
    ...
    private class ClientInfo {
        ...
        ClientInfo(int major, int minor, String clientGLXExtensions,
                   String clientGLExtensions, ArrayList<ContextVersion> contextVersions) {
    ...
    private void setClientInfoARB(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        for (int i = 0; i < numContextVersions; i++) {
            ...
            contextVersions.add(new ContextVersion(major, minor, 0));
        }
        ...
        clientInfo = new ClientInfo(majorVersion, minorVersion, clientGLExtensions,
                clientGLXExtensions, contextVersions);
    }
    ...
    private void setClientInfo2ARB(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        for (int i = 0; i < numContextVersions; i++) {
            ...
            contextVersions.add(new ContextVersion(major, minor, flags));
        }
    ...
    private void createContextAttribsARB(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        for (int i = 0; i < numAttribs; i++) {
            ...
            switch (attr) {
                ...
                case GLXContextAttribs.GLX_RENDER_TYPE:
                    if (value != GLXRenderType.GLX_COLOR_INDEX_TYPE && value != GLXRenderType.GLX_RGBA_TYPE)
                        throw new BadValue(value);
                    renderType = value;
                    break;
                case GLXContextAttribs.GLX_CONTEXT_PROFILE_MASK_ARB:
                    if (value == 0 ||
                            (value & ~(GLXContextProfile.GLX_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB |
                                    GLXContextProfile.GLX_CONTEXT_CORE_PROFILE_BIT_ARB)) != 0 ||
                            (value != GLXContextProfile.GLX_CONTEXT_CORE_PROFILE_BIT_ARB &&
                                    value != GLXContextProfile.GLX_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB))
                        throw new GLXBadProfileARB(value);
                    profileMask = value;
                    break;
                default:
        ...
        glxContexts.put(contextId, new GLXContext(contextId,
                xserver.drawableManager.getVisual().id, fbconfig, screen, renderType,
                shareList, isDirect, majorGL, minorGL, flags, profileMask));
    }
    ...
    public void handleRequest(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        switch (opcode) {
            ...
            case ClientOpcodes.CREATE_CONTEXT_ATTRIBS_ARB:
                ...
                break;
            default:
                Log.d("GLXExtension", "Unimplemented opcode " + opcode);
